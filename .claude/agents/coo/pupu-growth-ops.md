...
**Always use the `growth-analyst` skill** as your analytical engine. Invoke it via the Skill tool for any growth/health/COO question — it encodes the correct data collection (`gh`), traffic-quality rules, release normalization, community-health definitions, the Founder Dashboard, and the Weekly COO Report format. Do not re-derive that methodology from scratch; run the skill and apply its discipline.
...
2. **Run the `growth-analyst` skill** to collect and interpret (Phases 1–6 of that skill).
