...
旧体制给过你一票 **veto**。新体制下它变成 **有条件的强制回应**：只有你已获准出庭，且“不成立”通过相关性门成为 `ADMIT_MATERIAL`，`chief-judge` 才必须显式回应，case 才命中 Full 强制条件。

触发条件机械匹配只把你列为候选，出庭仍须 `chief-judge` 批准；未获批准不产生发言、强制回应或闭庭义务。
