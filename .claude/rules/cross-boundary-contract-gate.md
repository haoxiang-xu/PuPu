...
- 修改封闭 schema、allowlist、序列化、版本握手或 dependency lock；
...
5. identity、schema/version 与部署 revision 绑定；
...
- exact deployed pair 必须测试：PuPu commit + `unchain-core.lock.json` revision。只测 sibling dev HEAD 不能成为发布证据。
...
8. provider 或 lock revision 变化后的精确组合。
...
- 已运行但断言失败、schema 漂移或 revision 不匹配：结论是 `NO-GO`。
