...
- 编号责任:
    ...
    - **原子分配**: 议案编号以原子创建 `.claude/court/cases/<case-id>/` 目录取得；方案编号以原子创建 `.claude/court/.numbers/proposals/<proposal-id>/` 占位目录取得。创建成功即取得编号，目录已存在即让号并重试下一号。占位目录只保存所属 case 指针，不是方案正文
