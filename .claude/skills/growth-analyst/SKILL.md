...
```bash
mkdir -p .claude/archive/growth && D=$(date +%F)
gh api repos/$REPO/releases > .claude/archive/growth/$D-releases.json
gh api repos/$REPO/traffic/views > .claude/archive/growth/$D-views.json 2>/dev/null  # only way to keep >14 days
```
