...
**2 · legacy 的 `Isolated` 是三条轴的压平，不是一个状态的四种口味。** `not_root_run` ≙ `EnqueueDisposition.NO_OP`（enqueue 轴，意思是「根本没有 run」，不该是终态）· `root_run_{cancelled,failed}` ≙ `SourceRunStatus` · `capture_*` ≙ ~~`RunCaptureStatus`~~ **（2026-08-08 更正：至少 active 面读的是 `capture_quality`，其上游 typed 为 `ContextBuildStatus` 不是 `RunCaptureStatus`，故 `capture_legacy` 也可达，压平的轴比这里数的多一条。见 [[memory-v2-trace-vocabulary-anchor]] 第 2 条）**。只有候选级隔离才配叫 `Isolated`。**故「固化 reason 取值集合并入契约」= 把压平写进契约，且 `memory_v2_curator.py:484` 的 reason 是 `f"capture_{...}"` 字符串插值，集合天然开放，固化不了。**
