...
> **PARTIALLY SUPERSEDED（2026-08-14）：** 本文关于必须触发产品自身 import bootstrap、核对实际 `unchain.__file__` 的经验仍有效；其中 checkout/lock 三方对照已废止。当前 runtime compatibility 只认实际 import runtime 的 strict protocol manifest，release continuity 只认同一个已构建 wheel 的 SHA-256 与 manifest digest；Git revision/source 仅作遥测。
...
- 顺带闭合的一层：运行时 `import unchain` 的实际解析位置必须先被观察，再从该已加载runtime读取并严格校验protocol manifest；不能从期望路径、Git revision或checkout状态推断compatibility。发布验收另行核对所有consumer使用同一个immutable wheel artifact。

历史相关（已废止）：[[unchain-evidence-must-cite-lock-revision]]
