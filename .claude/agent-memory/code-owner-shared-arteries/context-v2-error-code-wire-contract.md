...
**消费者清单（`src/` 非测试，2026-08-08）**：`chat-bubble/memory_v2_journal_reload.js` · `chat-bubble/memory_v2_pending_reviews.js`（**唯一的 `listSpaces` 消费者，`:782`**）· `chat-bubble/memory_v2_trace_audit.js` · `PAGEs/chat/hooks/use_chat_stream.js:89`。**`getStatus` 与 `getTree` 各自零消费者，但 `getStatus` 已是承重的**——`resolveApi()` 要求 18 个方法 **全在** 才可用，缺任一个整个 facade 失明。
...
相关：[[memory-v2-trace-whitelist-topology]] · [[release-flag-state-is-not-in-the-repo]]
