...
boundary_revision_set: sha256:5dae9c3d1c6faf5cc1cfea8f5adf4687f0ab90965ed899191a7ab1a9419b5bf2+sha256:278149476c4ce35ce7ec53ce67866c5ce117d927f2d42e787f208cdb719d7878
updated_at: 2026-08-15T16:30:25-07:00
---
...
- **验收标准**:
  ...
  - AC-016 | fresh renderer hint absent或exact one-item≤1KiB closed schema；UTF-8 bytes严格来自`text.slice(0, composer.templateLength)`；generic/active fresh ingress均一致且无raw/name/id/hash。
  - AC-017 | sidecar以authoritative fresh message/composer evidence重算UTF-8 bytes；exact match才canonicalize并原子mint identical private durable option；invalid与mismatch分别返回`fresh_hint_invalid`/`fresh_hint_mismatch`，只丢composition且不持久化。
  - AC-018 | `_context_composition_hint_v1`进入stable durable resume option allowlist并在graph-step/resume byte-identical rehydrate；arbitrary component_state永不成为authority。
...
- **identity/version binding**: producer sha256:5dae9c3d1c6faf5cc1cfea8f5adf4687f0ab90965ed899191a7ab1a9419b5bf2 + consumer sha256:278149476c4ce35ce7ec53ce67866c5ce117d927f2d42e787f208cdb719d7878
...
- **canonical representation**: existing versioned RunBundle/bundle_json/runtime event carriage；composition只存在于provider receipt namespaced extension；runtime features为availability-only；unavailable reason exact `{schema:'pupu.context_composition_availability.v1',code:<closed-enum>}`且无details
...
- **identity/version binding**: producer sha256:5dae9c3d1c6faf5cc1cfea8f5adf4687f0ab90965ed899191a7ab1a9419b5bf2 + consumer sha256:278149476c4ce35ce7ec53ce67866c5ce117d927f2d42e787f208cdb719d7878
- **producer owner confirmation**: HS-002
...
- **consumer**: PuPu sidecar fresh-message/composer authoritative validator
- **consumer owner**: code-owner-runtime
- **canonical representation**: absent或≤1KiB exact `{schema:'pupu.context_composition_hint.v1',contributions:[one exact {category:'skills',subtype:'expanded_invocation',surface:'messages',utf8_bytes:positive-safe-int,source_count:positive-safe-int}]}`；无extra/name/raw/id/hash
- **consumer projection**: renderer用`TextEncoder(UTF-8).encode(text.slice(0, composer.templateLength)).length`；sidecar从authoritative fresh message/composer evidence独立重算，only exact match canonicalizes
- **admission policy**: CLOSED
- **admission details**: fresh only；exact top/item key set、single item/fixed enums、safe integer、≤1KiB；generic/active fresh ingress一致；absence表示no hint
- **unknown input behavior**: invalid=`fresh_hint_invalid`，authoritative recompute mismatch=`fresh_hint_mismatch`；discarded for composition only，不进入durable state，不改变provider request/send
- **failure semantics**: ordinary V4 fresh send继续；只产生stable content-free composition unavailable diagnostic；不猜bytes或subtype
- **identity/version binding**: producer sha256:5dae9c3d1c6faf5cc1cfea8f5adf4687f0ab90965ed899191a7ab1a9419b5bf2 + consumer sha256:278149476c4ce35ce7ec53ce67866c5ce117d927f2d42e787f208cdb719d7878
- **producer owner confirmation**: HS-006
...
- **consumer owner**: code-owner-unchain
- **canonical representation**: admitted public hint的byte-identical content-free private value；fresh atomic mint；resume absence/equality/conflict三态；无arbitrary component_state authority
...
- **owner confirmation**: HS-004
- **identity key**: fresh turn request + authoritative message/composer evidence + canonical hint bytes
- **initial state**: fresh renderer request尚未sidecar admission，durable private hint不存在
- **ordered events**: renderer absent/exact/invalid hint → sidecar strict schema → authoritative UTF-8 recompute → exact match atomic private mint或composition-only discard → ordinary V4 send
- **expected observations**: absence不mint；exact single item mint byte-identical private value；invalid/mismatch不persist；两条fresh ingress相同；provider send不受composition availability影响
