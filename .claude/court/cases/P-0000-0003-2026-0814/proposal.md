...
updated_at: 2026-08-14T09:50:00-07:00
---
...
- **关键步骤与依赖**:
  1. Unchain 实际加载模块导出 closed manifest 与可重算 digest，不读取 Git 或外部 capability JSON。
...
- **验收标准**:
  - AC-001 | 实际加载的 Unchain manifest在Python严格round-trip，digest mutation、重复/乱序、未知core字段均拒绝。
  - AC-002 | sidecar与Electron对同一真实producer manifest独立通过；缺协议、错误major、过低minor、缺feature、坏digest/shape均在side-effect前拒绝。
...
- **canonical representation**: `contracts/unchain-runtime-protocol-producer-v1.json`冻结的closed manifest、排序和domain-separated digest
...
- **admission details**: 顶层与item key精确；protocol/features唯一规范排序；digest独立重算；未知额外protocol/feature允许但不满足required项
