...
### 独立首轮提交 · 补交（`expert-architecture`）

> 归档说明：本份 `ASSESSMENT` 经 **四次派遣** 方得交付。前三次零产出（第 1/2 次 `rate_limit`，第 3 次 Fable 5 硬配额，见 E-0115 / E-0116），第四次依 `ruling.md#R-0001` 的授权改派 `opus`。第四次亦曾在 715KB transcript 后 **600 秒无进展停止**（transcript 内 `is_error` 与 error 字段均为 0，**无任何结构化错误签名**，与前三次成因不同），但因任务书要求「中途先落一版盘」，其 v0 草稿与 `E-0090` / `E-0091` 两份证据已先行落盘未灭失；本庭据此以 `SendMessage` 续跑同一 instance（保留其 715KB 取证上下文），令其停止取证、就地补全五个空缺字段，遂得本份完整发言。**本庭已校验公共信封五字段与 `Expert` 三字段齐备，未作任何改写。** 至此 **法定必到 12 人全部交付，quorum 满。**
...
- **阶段**: 议案庭审
- **结论**: 传唤第三层集合差检查 **第二次执行**（S-0014 交付后的完整案卷）：新引入实体的 owner **全部在场**；两处新增路径命中判定 **无需补行传唤**，理由随附
- **依据**: S-0014；E-0090；E-0091；E-0104；[summons 第三层](../../../codex/lifecycle/summons.md)
- **不确定性**: 无
- **请求/下一步**: 本条为闭庭门禁第 3 项（传唤第三层）的最终执行记录
- **通知类型**: PROCEDURAL
- **生效时间**: 2026-08-07T23:55:00-07:00
- **影响范围**: 闭庭条件

**一 · S-0014 新引入的实体，owner 均在场**

`src/SERVICEs/boot_readiness.js` → `code-owner-shared-arteries`（charter 逐字声明该文件）**在场**（S-0008）；`electron/main/services/boot_readiness/**` → `code-owner-electron`（`pupu:electron/**`）**在场**（S-0010）。

**二 · 两处新增路径命中，判定无需补行传唤（本庭作出，理由公开备 `codex` 复核）**

`src/COMPONENTs/agents/pages/recipes_page/recipe_canvas.js`（→ `code-owner-agents`）与 `src/COMPONENTs/toolkit/plugins_shell.js`（→ `code-owner-toolkit`）**仅出现于 `code-owner-ui-primitives` 的 E-0104 中的一张统计表** —— 该表列举全仓 100 个 icon-only `<Button>` 调用点里 **81 个未传 `ariaLabel`** 的分布，上述两文件各占 3 个。

判定 **无需补行传唤**，三条理由：

1. **它们不是本案的实体，是关于 `Button` 这个原语的数据点。** 该证据的论证对象与结论落点都是 `src/BUILTIN_COMPONENTs/input/button.js`，其 owner `code-owner-ui-primitives` **在场**，且该证据正是它本人提交的
2. **本案不对这两个文件提出任何改动。** E-0104 自身的结论是「焦点不可见是 **原语级缺陷**，不是调用方能修的」「**必须修在原语层**」—— 处置落点在场内
3. **若统计表的每一行都要求其 owner 到场，任何全仓普查都会自我否定** —— 取证越彻底，传唤面越宽，直至无法闭庭。这会把第三层从「漏人检测」变成「取证惩罚」，与其设立目的相反

**三 · 由此浮现一项超出本案范围的发现，登记为 side case 候选（本庭不代立案）**

**全仓 100 个 icon-only `<Button>` 中 81 个未传 `ariaLabel`（81%），分布跨至少 8 个文件、5 个 owner**；且 `code-owner-ui-primitives` 自陈其边界内的 `title_bar.js` 4 个（窗口最小化/最大化/关闭/恢复）**自己也没传**。这是一条 **全仓可访问性债**，与本案题面（记忆的用户可见面）**只是同族不同题**。连同 `expert-security` 建议的两个非阻塞侧案（孤儿 scope 主进程侧对账清扫、sidecar 错误归一化审计 109 处 `str(exc)`）、`code-owner-devtools` 建议的发布 flag 决定点侧案、以及 `code-owner-shared-arteries` 报出的 E-0039（custom provider 送信路径 6 个幽灵 locale 键，与 Memory 无关），一并交 `chief-judge` 决定另立还是并入既有批次。

---

## 闭庭

#### S-0024 | SUMMARY | speaker-of-the-house → case
- **阶段**: 议案庭审
- **结论**: 闭庭门禁八项已逐项确认满足，宣布闭庭，提交以下忠实汇总
- **依据**: S-0001 … S-0023；E-0001 … E-0116；`ruling.md#R-0001`
- **不确定性**: 本案 **全体 12 名必到者均自陈无法就「正常态」取证**（本机 official store `entries=0`）。故本汇总中一切与「记忆有数据时用户看到什么」相关的内容 **均无实证支撑**，仅空态与失败态有据
- **请求/下一步**: 提交 `Chief Judge` 进行议案裁定。**强制回应事项 16 项须逐条显式回应方可裁定**（闭庭门禁第 8 项）
- **共识**: S-0005, S-0006, S-0007, S-0008, S-0009, S-0010, S-0011, S-0012, S-0013, S-0014, S-0015, S-0016
- **分歧**: S-0005↔S-0001 · S-0007↔S-0001 · S-0008↔S-0010 · S-0009↔S-0007 · S-0013↔S-0005/S-0006/S-0008/S-0010 · S-0014↔S-0007/S-0011 · S-0015↔S-0012
- **已知缺口**: E-0115, E-0116, S-0003, S-0017, S-0023
- **候选方案**: 无
- **风险**: E-0056, E-0057, E-0064, E-0081, E-0114, S-0014 正文五
- **强制回应事项**: S-0012（6 项 + 1 预防性）· S-0013（5 项）· S-0014（5 项）
...
| # | 门禁 | 确认 |
|---|---|---|
| 1 | 必到角色均已提交职责要求的输出 | **满足**。法定必到 **12 人全部交付**（立案 6 → 第一层复核 10 → 第三层补 2）。`expert-architecture` 经四次派遣、一次 `SendMessage` 续跑方得交付（S-0021、R-0001、E-0115、E-0116） |
| 2 | 每项具证明力的事实主张都有证据编号，每项证据都有验证状态 | **满足**。**91 条证据**均登记验证状态。**全案未传唤 `evidence-examiner`**，故除 E-0111、E-0116 外均为「未验证 / 首次提交」—— 见下「证据强度声明」 |
| 3 | 每个 material `QUESTION` 已获答或列为已知缺口 | **满足**。三条立案已知缺口中 **两条已了结**（E-0011、E-0020），一条结构性保留；庭中新生的两个 material 争点（`store_owner` 现网取值、`SQLiteContextV2StoreReadStatus` 字段集）**均已由定向质询与第三层补传唤了结**（E-0056、E-0110/E-0111） |
| 4 | 所有 blocking `Witness` 传票均已回应 | **不适用**。本案 **未签发任何 `Witness` 传票** —— 无任何出庭角色提出满足事实缺口四项门禁的传唤请求 |
| 5 | blocking 性质争议均已有 `Procedural Judge` 裁定 | **不适用**。无传票，无争议 |
| 6 | 每项 `OBJECTION` 已标记 | **不适用**。本案 **未提交任何 `OBJECTION`** 动作。全部反对意见以 `Expert` 的「不成立」与 `Code Owner` 的直接反驳形式出现，已分别进入强制回应清单与分歧栏 |
| 7 | 可供裁定的方案彼此可区分且带风险/可逆性/验收标准 | **不适用**。**议案庭审阶段不产生方案**，12 名必到者 **无一提交 `PROPOSAL`**，符合阶段要求 |
| 8 | `Expert` 的「不成立」与 `Dimension Owner` 的「反对」已进强制回应清单 | **满足**。三名 `Expert` 合计 **16 项「不成立」**（+1 预防性）已全部录入。**无 `Dimension Owner` 在案** —— 其评估对象为「组织变更议案」，本案为产品议案，评估对象未命中，[summons 第一层的 `Dimension Owner` 例外](../../../codex/lifecycle/summons.md) 不触发。**依该条，本庭注明：本案四个组织维度均未被覆盖**，且这不构成缺席 |

**证据强度声明（本庭必须前置说明，否则裁定会高估案卷）**：本案 **未传唤 `evidence-examiner`**（议案庭审阶段，证据未被用于不可逆 action 的准入判断）。故 91 条证据中绝大多数为提交方自陈的只读检查，**验证状态为「未验证」**。其中 **仅两条经第二方独立核实**：E-0111（`code-owner-unchain` 三方对照 lock revision 与工作树）与 E-0116（本庭直接收到的通知原文）。另有数条构成 **交叉复核**（E-0091 独立复核 E-0030；E-0104 加强 E-0072/E-0049；S-0016 复核 E-0027），其证明力高于单方自陈但仍未经审查。**任何据本案证据作出的不可逆授权，应先补证据审查。**

---

### 一 · 共识

**C1 · 四态今天在每一层各自独立地塌缩**，且塌缩形态各不相同 —— 服务端单端点（E-0022/E-0023）、整个 V2 只读面（E-0024）、Electron main 就绪闸（E-0031）、Inspector（E-0002/E-0003）、气泡（E-0045）、文案（E-0035/E-0037）、unchain 上游（E-0114）。**12 名必到者无一反对该事实描述。**

**C2 · 议案自带约束「新 Inspector 不得继承 200-空成功形状」成立，但写窄了。** `code-owner-runtime`（C2）主张扩为「**任何被 Inspector 用来判断四态的读端点，其 200 body 必须带一个显式的状态判别位**」；`expert-architecture` **采纳该扩写**（但另判其「已充分」不成立，见强制回应 ③）。

**C3 · Q2/Q5 的分流落在 modal 内部（`memory_inspect_modal.js`），入口保持 admission-blind。** `code-owner-chat-core` 主张并自陈越界主动交出（S-0006）· `code-owner-settings` 承接（S-0005）· `expert-ux` 判落点成立（S-0012）· `expert-architecture` 判成立（S-0014）。

**C4 · 挂载接口须补 `ownerChatId`，且破坏面为零。** `code-owner-chat-core` 已核实两个分支的取值均已在调用点手上（E-0016/E-0017），**本端承诺该扩展**；`code-owner-shared-arteries` 复核并加强其举证（E-0033）。

**C5 · `/context/v2/status` 不开 counts。** `code-owner-runtime`（D5，scope 泄漏 + 上游结构性缺席）· `code-owner-unchain` **逐字复核确认**（E-0110/E-0112）· `code-owner-devtools` **接受且主动放弃**其可主张的便利。三方独立同向。

**C6 · 不新增 `listArtifacts`。** `code-owner-runtime` 维持前案结论并补成立条件（错误侧须可分辨）· `code-owner-chat-bubble` **不主张新增**。

**C7 · Q4-B 落点为 `settings > local_storage`（非 memory 页）。** `code-owner-settings` 承接 · `code-owner-chat-core` 拒接（正式表态，非参考）· `code-owner-runtime` 不落其端（E-0021）· `expert-architecture` 判成立 · `expert-ux` 与 `expert-security` 有条件成立。

**C8 · 本仓已有一份跑通并守住的四态先例可复用** —— `boot.*` / `boot_readiness`（E-0038 文案半边、E-0090 结构半边，两者不重叠）。`expert-ux` 与 `expert-architecture` 独立指向同一先例。

---

### 二 · 分歧（**原样保留，未压平**）

**D1 · 议案框定的两处事实陈述被必到者更正**（本庭自陈：这两条是本庭 S-0001 从前案继承而来，未经复核即写入框定）
- 「`/memory/projection` 在**所有**失败路径返回 200」→ **过陈述**。`code-owner-settings`（E-0004）与 `code-owner-runtime`（E-0022，文件 owner）独立更正为：**承载状态语义的失败（为空 + 四种未就绪）返回 200 空，基础设施级异常返回 500**。**修法方向因此完全不同**（不是「把 200 改成错误码」，而是「让 200 自己说清楚它是哪一种 200」）
- 「`get_session_head` 已实现三路判别」→ **实为四路**，且 `200` 本身不是单一态，前端 **必须再读 `mutation_ready`**（E-0020）。`code-owner-runtime` 就此 **主动更正其本人前案发言**
- **`code-owner-chat-core`（S-0006 正文四）仍沿用旧表述**（「在所有失败路径都 200，等于零信号」）。该表述与 E-0022 不一致，**本庭不代为修改**，如实并列

**D2 · 四态判据的形状** —— 框定说「三路」；`code-owner-settings` 核到 **至少 8 个码且随 `store_owner` 变形**，据此立 **C2「判据默认拒真」**（任何三分支 switch 都会把未覆盖的码默默落进 else）；`code-owner-runtime` 核到 **四路 + 第二判别轴**。三者结论方向一致（不需要新契约）但 **理由互不相同**。

**D3 · 压平点的归因** —— `code-owner-shared-arteries` 判「压平在 Electron main 就绪闸，11 原因塌成 1 码」（E-0031）；`code-owner-electron` 判「**IPC 层没有压平**，CONTEXT_V2 路上 stable code 端到端不丢，唯一例外是 `/memory/projection`」。**二者对「默认构建 `effectiveMode === "off"` 时请求照发、命中 404」的结论一致**，但对「压平发生在哪、是否构成四态的物理阻碍」归因不同。`code-owner-electron` 另更正 `code-owner-settings` 的机制表述（「结论对，但不是 readiness gate 掐死的」）。

**D4 · Q7 是不是被问错了** —— `code-owner-chat-bubble` 判 **「Q7 问错了，这是本发言最实质的一条」**（两个选项对用户可见面净效果都是零，因变量在气泡侧）· `expert-ux` 与 `expert-architecture` 同意改判 · 而 `code-owner-runtime` 维持其契约层结论。**这不是矛盾，是两个层面的问题被并成一问**；`expert-architecture` 判「Q7 的提问顺序」不成立（强制回应 ④）。

**D5 · 对比度阶梯** —— `expert-ux`（E-0074）给出两档取值；`code-owner-ui-primitives`（第三层补入者）**实测推翻其次级档**（9 套出厂调色板上最差 **3.69:1，不通过**），并更正其三条证据的输入取值（生产环境 `theme.semantic` 恒满、modal 底 dark 实为 `#121212`）。**`expert-ux` 的判定结论未被推翻，但其可实施的取值阶梯被推翻一档。**

**D6 · `expert-security` 推翻四名 `Code Owner` 的四条主张** —— 见强制回应 ①②③④⑤。其中最重的一条：**`code-owner-settings` 的 C7 若按字面执行，Q4-B 的撤销入口永久不可实现**（全仓无任何方法返回 `grantId`，`revoke` 字面不可调用）。

**D7 · 本案六问的划分是否正确** —— `expert-architecture` 判 **有条件成立**：六问在「用户看到什么」这一层切分正确，但 **缺一个前置问 Q0**（是否该有单一状态源）。其核心观察：**四名角色在互不知情的独立首轮里，各自索要了同一个不存在的构件**（公共就绪度读 / 正确的就绪探针 / 可复用判定函数 / 吸收 code 增长的中间层），**而它今天没有 owner**。

**D8 · 一处「三方主张全部正确、合起来仍无人负责」的结构缺口** —— Q2/Q5 的接缝（位置参数换对象、chatId 与 sessionId 拆两字段）跨 `code-owner-chat-core` / `code-owner-settings` / `code-owner-shared-arteries`。`expert-architecture` 明言 **「传唤机制解不了它，只有指派能解」**，请求 `chief-judge` 直接指派 owner。

---

### 三 · 强制回应事项（16 项 + 1 预防性，**须逐条显式回应方可裁定**）

**`expert-ux`（S-0012）· 6 项**：① fail-closed 继续放 disclaimer 槽位（11px / **2.11:1 dark · 2.42:1 light**，连 3:1 都不到）· ② 「加 `title` 即 blocked 最小可接受形态」（action bar 本身 `opacity: hovered ? 1 : 0`，**等于把解释藏在解释之后**，键盘/触控够不到）· ③ `retryable` 两分类作为完整文案分类（缺「卡住」这一类，**而那是唯一配得上按钮的**）· ④ 保留「Please try again shortly」（就绪判定是 boot 快照永不刷新，**把用户送进无限循环**）· ⑤ reason code 句内插值 · ⑥ `ArtifactSummary` 在 `snapshot` 缺失时渲染字面量 `{}`。**另 1 项预防性**：按 admission 结果改变 `Inspect Memory` 菜单项的出现与否/顺序（上下文菜单条目集合须位置稳定，相邻有破坏性操作）。

**`expert-security`（S-0013）· 5 项**：① `code-owner-settings` **C7 的字面表述**（会永久锁死 Q4-B 的撤销入口）· ② `code-owner-shared-arteries`「新增枚举推翻 P0 不变量」（渲染进程 **已同步持有** `store.chatsById` 全量，该不变量实为「**一次调用**不可枚举」；连带「全量清单做不出来」也不成立，是性能问题非契约问题）· ③ `code-owner-electron` D-4「含 Traceback」（送的是 `str(exc)`；全仓 `format_exc()` **仅三处且全部只进 stderr**）· ④ `code-owner-chat-core`「版本号/UUID 误报是高频骚扰」（扫描器双重锚定，裸版本号/UUID **结构上不命中**，且 **从未有人测量过误报率**）· ⑤ `code-owner-chat-core`「逃生口属安全语义变更」（该 fail-closed 防的是 **记忆一致性**，无攻击者模型、无权限边界 —— **并非所有 fail-closed 都是安全控制**）。

**`expert-architecture`（S-0014）· 5 项**（**该角色明确要求保留每项的限定半句，本庭照录**）：① `code-owner-devtools` D-2 的 **落点**（**不否定要一个共享判定，那是对的；只否定从 `memory-inspect/**` 导出这个落点** —— 会把 Inspector 变成事实公共依赖，压破 C8）· ② **Q2/Q5 接缝的现状归属**（**针对现状，不针对任何一方发言；三方边界主张全部正确**）· ③ `code-owner-runtime` C2 的 **充分性**（**采纳 C2，只否定它已充分** —— 只覆盖 200 body，不覆盖 error code 枚举完整性与 renderer 未知码兜底，E-0032 是判别位解决不了的反例）· ④ **Q7 的提问顺序**（两个自变量取值映到同一因变量值，**是在因变量不敏感的区间上做选择**）· ⑤ **Q4-B 的显示时序**（**与 `expert-ux` C-UX-4(c) 同向加强，非独立发现，可合并处置**）。

**`Dimension Owner` 反对：无。** 四个组织维度 **均未被覆盖**，理由见闭庭门禁第 8 项。
...
| # | 缺口 | 状态 |
|---|---|---|
| 1 | 本机 official store `entries=0` / `candidates=0` | **仍开，结构性。12 名必到者全部自标「正常态给不出取证」** —— 这是本案最大的证据空洞 |
| 2 | `streamError` 清除点未穷举 | **已了结**（E-0011）。结论比原猜测更坏：4 个清除点，`use_chat_session_state.js:440` **每次会话切换无条件清空且无回放**；跨会话路径上该提示 **连「显示过」都不成立** |
| 3 | `get_session_head` 空 `session_id` 行为 | **已了结**（E-0020，实测）。三个 store owner 三个答案（400/404/503），**没有一个表达 admission** |
| 4 | 传唤第二层以缩减形式执行 | **仍开，程序缺口**（S-0003）。覆盖「抽取漏掉」与「议案写窄」，**未覆盖「边界写窄、须 agent 本人辨认」** |
| 5 | `/.local/build_feature_flags.snapshot.json` **无 owner** | **仍开，须 `chief-judge` 明示**（S-0017）。git-ignored、31 份 charter 无一覆盖、不在 A-009 内，却是 **发布构建的输入** |
| 6 | `summon.py` 与绝对路径纪律互相抵消 | **仍开**（S-0017）。**成因在本庭自己** —— 本庭要求出庭者用绝对路径，而工具 glob 是仓内相对路径。A-010 同类缺陷 **第四次发作** |
| 7 | A-012 无法区分三种失败签名 | **仍开**（S-0021）。本案单一角色即产生 **三种可区分签名**：`rate_limit`（并发度 2）· 硬配额（并发度 1、等待 40 分钟后仍触发）· **600 秒无进展且零结构化错误**（第四次）。三者处方互不相同 |

---

### 五 · 风险

**R1 · 单向门**（`expert-architecture` S-0014 正文五）—— 其中第 4 项经该角色请求单列：**本案凡以「今天默认 store owner 的行为」为据的结论，在 owner 切换当天全部需要重核。** 本案有大量结论建立在 `store_owner === "off"` 这一现状上（E-0056）。

**R2 · 发布 flag 分叉（时间敏感，独立于本案裁定成立）** —— `enable_memory_v2` 在已发布安装包里 **不是 false 而是「不存在」**，而本机 `.local` 快照现为 `true`：**下一次发布构建会烤进一个与线上分叉的包**（E-0057、E-0064）。

**R3 · C7 若按字面执行，Q4-B 的撤销入口永久不可实现**（E-0081）。「想撤销没有入口」是本案 Q4-B 的立问前提，**而按当前 C7 表述它是一个按定义无解的问题**。

**R4 · 跨仓契约钉死点** —— `SQLiteContextV2StoreReadStatus` 是 `frozen=True, slots=True` **单一构造点、无扩展点**；任何加字段都是跨仓契约变更，且 `unchain-core.lock.json` 的 revision 须同步推进。今天 lock 与 unchain `dev` HEAD **恰好相等，此巧合不可依赖**（E-0111、S-0016 C-U2）。

**R5 · 优先级倒序的返工风险** —— `expert-ux` 与 `expert-architecture` 均有条件同意「先做 V1」，但 `expert-architecture` 判 `expert-ux` 的条件 **必要但不充分**：须由 **import 强制**（唯一权威模块 + 测试从该模块 import 而非转写 + 断言权威模块覆盖全部可发出的码），**三条缺一，倒序必返工**。

---

### 六 · side case 候选（本庭不代立案，交 `chief-judge` 决定另立或并入既有批次）

1. **发布 flag 决定点**（`code-owner-devtools`）—— 对应 R2 与已知缺口 5
2. **孤儿 scope 主进程侧对账清扫**（`expert-security` 推荐为占优方案：差集直接删、只暴露计数，**渲染进程不获得任何新枚举能力**）
3. **sidecar 错误归一化审计**（`expert-security`，**109 处 `str(exc)`**；其本人拒绝由一个样本给整类定级）
4. **全仓 icon-only `<Button>` 可访问名缺失**（`code-owner-ui-primitives` E-0104：**100 个调用点中 81 个未传 `ariaLabel`**，跨 5 个 owner；且原语层 `outline:none` 写死、无 focus 通道，属 **结构性无出口**）
5. **E-0039 幽灵 locale 键**（`code-owner-shared-arteries`：custom provider 送信路径 **6 个从未存在的键**，与 Memory 无关，属既存 i18n 断裂）

---

### 七 · 本庭自身的失范留痕（一并上呈，不隐去）

本案审理中，**主持人有一次证据捏造**：S-0018 在 **零观测** 下断言 `expert-architecture` 的失败成因并逐字捏造错误串，据此建立了一整节 A-012 论证。已按协议以 S-0019 撤回、S-0020 替代，旧记录保留并标注失效。**第三次派遣后证明该断言内容碰巧正确，本庭明确不予追认**（理由见 S-0021 第二节）。

**该错误的发现纯属偶然** —— 一次监视器超时迫使本庭复核落盘状态。**若无那次超时，这条捏造的证据会随本 `SUMMARY` 一起进入裁定材料。** 此后本庭对每一条失败断言均先取结构化证据再归档（E-0115、E-0116），并在第四次派遣时避免了同类错误的第二次发生（监视器的文本匹配命中了本庭自己写进任务书的错误串，构成假阳性，经结构化字段核验后否定）。

**本庭请 `codex` 就此次失范一并审查。** 归档而非无声修补的理由：**主持人是本案唯一不受其他角色审查的环节**，案卷若被无声修补，`codex` 的合法性监督即失效。
