...
import { resetComputerUsePreferencesForTests } from "../../../SERVICEs/computer_use_preferences_sql";
...
const installFailingConsentApi = () => {
...
const ResetHarness = () => {
  const { resetConsent, consentRecord } = useComputerUseConsent();
  const [result, setResult] = useState("pending");
  return (
    <div>
      <button
        onClick={async () => {
          setResult(String(await resetConsent()));
        }}
      >
        reset
      </button>
      <div data-testid="reset-result">{result}</div>
      <div data-testid="reset-record">{consentRecord ? "has" : "none"}</div>
    </div>
  );
};
...
describe("useComputerUseConsent gate", () => {
  ...
  test("SQL persistence failure resolves false and never exposes consent", async () => {
    installFailingConsentApi();
  ...
  test("reset disables the running sidecar before clearing consent", async () => {
    window.localStorage.setItem(
      "computer_use_consent",
      JSON.stringify({ version: 1, acceptedAt: new Date().toISOString() }),
    );
    withProviders(<ResetHarness />);
    ...
    expect(screen.getByTestId("reset-record")).toHaveTextContent("none");
    expect(readComputerUseConsent()).toBeNull();
  });

  test("reset keeps consent while disable is pending, then clears it", async () => {
    window.localStorage.setItem(
      "computer_use_consent",
      JSON.stringify({ version: 1, acceptedAt: new Date().toISOString() }),
    );
    const disabling = deferred();
    disableComputerUse.mockReturnValue(disabling.promise);
    withProviders(<ResetHarness />);
    ...
    expect(screen.getByTestId("reset-result")).toHaveTextContent("pending");
    expect(readComputerUseConsent()).not.toBeNull();

    disabling.resolve({ pushed: true, ok: true, enabled: false });
    await waitFor(() =>
      expect(screen.getByTestId("reset-result")).toHaveTextContent("true"),
    );
    expect(readComputerUseConsent()).toBeNull();
  ...
  test("reset retains consent when the sidecar cannot be confirmed off", async () => {
    window.localStorage.setItem(
      "computer_use_consent",
      JSON.stringify({ version: 1, acceptedAt: new Date().toISOString() }),
    );
    ...
    expect(screen.getByTestId("reset-record")).toHaveTextContent("has");
    expect(readComputerUseConsent()).not.toBeNull();
  });
});
