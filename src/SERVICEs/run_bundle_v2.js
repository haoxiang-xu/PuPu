...
export const RUN_BUNDLE_DETAILS_REF_SCHEMA =
  "unchain.run_bundle_details_ref.v1";
export const RUN_BUNDLE_V2_MAX_BYTES = 512 * 1024;
export const RUN_BUNDLE_V2_MAX_DETAILS_BYTES = 64 * 1024 * 1024;
...
export const canonicalizeRunBundleV2 = (value) =>
...
const requireNullableText = (value, label) => {
...
const normalizeDetailsRef = (value) => {
  ...
  if (value.total_bytes > RUN_BUNDLE_V2_MAX_DETAILS_BYTES) {
    throw errorWithCode(
      "bundle.details_ref.total_bytes exceeds the durable limit",
    );
  }
  if (
  ...
  value.parts.forEach((part, index) => {
    exactKeys(
      part,
      ["name", "item_count", "canonical_bytes", "root_sha256"],
      `bundle.details_ref.parts[${index}]`,
    );
    requireText(part.name, `bundle.details_ref.parts[${index}].name`);
    if (names.has(part.name)) {
      throw errorWithCode("bundle.details_ref.parts names must be unique");
    }
    names.add(part.name);
    requireCount(
      part.item_count,
      `bundle.details_ref.parts[${index}].item_count`,
    );
    requireCount(
      part.canonical_bytes,
      `bundle.details_ref.parts[${index}].canonical_bytes`,
    );
    requireDigest(
      part.root_sha256,
      `bundle.details_ref.parts[${index}].root_sha256`,
    );
  });
  return value;
};
...
/**
 * Synchronous renderer admission for RunBundle v2. The browser validates the
 * locked shape, digest format and byte ceilings; Electron recomputes the body
 * digest at the durable write boundary, matching the v1 trust boundary.
 */
export const normalizeRunBundleV2 = (rawBundle) => {
  exactKeys(
    rawBundle,
    [
      "schema",
      ...
      "descriptor",
      "provider_call_count",
      "direct_provider_call_count",
      "descendant_provider_call_count",
      "aggregation_usage",
      "direct_usage",
      "descendant_usage",
      "metrics",
  ...
  if (rawBundle.schema !== RUN_BUNDLE_V2_SCHEMA) {
    throw errorWithCode("schema is unsupported");
  }
  ...
  ["execution_id", "attempt_id", "root_run_id", "run_id", "relation"].forEach(
    (field) => requireText(rawBundle.identity[field], `bundle.identity.${field}`),
  );
  requireNullableText(
    rawBundle.identity.parent_run_id,
    "bundle.identity.parent_run_id",
  );
  if (
    !["root", "subagent", "graph_node", "recipe_node", "auxiliary"].includes(
      rawBundle.identity.relation,
    )
  ) {
    throw errorWithCode("bundle.identity.relation is unsupported");
  }
  ...
  requireNullableText(
    rawBundle.lifecycle.completed_at,
    "bundle.lifecycle.completed_at",
  );
  requireNullableText(
    rawBundle.lifecycle.continued_from_run_id,
    "bundle.lifecycle.continued_from_run_id",
  );

  exactKeys(
    rawBundle.descriptor,
    ["model", "display_model", "active_agent", "agent_orchestration", "iteration"],
    "bundle.descriptor",
  );
  ["model", "display_model", "active_agent", "agent_orchestration"].forEach(
    (field) =>
      requireText(rawBundle.descriptor[field], `bundle.descriptor.${field}`),
  );
  ...
  [
    ...
    if (!isObject(rawBundle[field])) {
      throw errorWithCode(`bundle.${field} must be an object`);
    }
  });
  [
    "provider_call_count",
    "direct_provider_call_count",
    "descendant_provider_call_count",
  ].forEach((field) => requireCount(rawBundle[field], `bundle.${field}`));
  if (
    rawBundle.direct_provider_call_count +
      rawBundle.descendant_provider_call_count !==
    rawBundle.provider_call_count
  ) {
  ...
  if (
    utf8Bytes(canonicalizeRunBundleV2(body)) > RUN_BUNDLE_V2_MAX_BYTES ||
    utf8Bytes(canonicalizeRunBundleV2(rawBundle)) > RUN_BUNDLE_V2_MAX_BYTES
  ) {
...
export const isRunBundleV2 = (value) =>
