...
function buildFakePins() {
  ...
  const apacheUrl =
    "https://example.test/uv/0.1.0/licenses/LICENSE-APACHE";
        ...
        python: {
          ...
          post_stage: {
            ...
            copy_directories: [
              {
                from:
                  "lib/python3.12/site-packages/pip-26.1.2.dist-info/licenses",
                to: "licenses/pip",
              },
            ],
...
test("versioned pins cover every supported target without dynamic URLs", async () => {
  ...
  assert.equal(
    pins.runtimes.uv.default_env.UV_PYTHON_DOWNLOADS,
    "never"
  );
  ...
  assert.equal(
    pins.runtimes.python.default_env.PYTHONDONTWRITEBYTECODE,
    "1"
  );
...
test("Electron build scripts stage the matching target and package only runtime outputs", async () => {
  ...
  assert.match(
    packageJson.scripts["build:unchain:win"],
    /--target win32-x64/
  );
  assert.match(
  ...
  assert.deepEqual(
    packageJson.build.extraResources.slice(0, 2).map(({ from, to }) => ({
      from,
      to,
    })),
    [
      ...
      {
        from: "unchain_runtime/mcp_runtime",
        to: "mcp_runtime",
      },
    ]
  );
...
test("verification catches staged runtime tampering", async () => {
  ...
  await fs.appendFile(path.join(outputDir, "node", "bin", "node"), "changed");
...
test("afterPack verifies the runtime copied into app resources", async () => {
  const directory = await makeTempDir();
  const { pinsPath, payloads } = await writeFakePins(directory);
...
test("afterPack accepts Windows signing mutations without weakening staging verification", async () => {
  ...
  await fs.appendFile(path.join(outputDir, "node", "bin", "node"), "signed");
...
test("afterPack executes every bundled runtime natively without mutating it", async () => {
  ...
  await verifyPackagedMcpRuntime(
    ...
    { pinsPath, executeFile, requireNativeSmoke: true }
  );
