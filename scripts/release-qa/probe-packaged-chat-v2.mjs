...
// Usage: node scripts/release-qa/probe-packaged-chat-v2.mjs <fixed-candidate-directory>
if (process.platform !== 'win32') throw new Error('This probe requires Windows.');
if (process.argv.length !== 3) throw new Error('Usage: node probe-packaged-chat-v2.mjs <fixed-candidate-directory>');
...
const root = fs.mkdtempSync(path.join(candidateRoot, 'probe-packaged-chat-v2.mjs-'));
console.log(`Evidence directory: ${root}`);
const profile = `${root}/真实聊天 v2 private profile`;
...
async function launch(label) {
  ...
  log = fs.openSync(`${root}/chat-v2-${label}-app.log`, 'w');
...
async function chat(page, chatId, label, text) {
  ...
  fs.writeFileSync(`${root}/packaged-chat-v2-evidence.json`, JSON.stringify(report, null, 2));
