...
def _get_runtime_config() -> Dict[str, str]:
    provider = os.environ.get("MISO_PROVIDER", "ollama").strip().lower() or "ollama"
    model = os.environ.get("MISO_MODEL", _provider_default_model(provider)).strip() or _provider_default_model(provider)
    return {
        "provider": provider,
        "model": model,
...
def _build_payload(provider: str, options: Dict[str, float]) -> Dict[str, float]:
