...
async function probe(e){
 const id=e.toolkitId;let j=JSON.parse(fs.readFileSync(out+'/subjects/'+id+'.json'));if(id!=='mcp.workspace.markitdown')return;
 ...
 if(e.mcp.transport==='http'){transport=new StreamableHTTPClientTransport(new URL(e.mcp.url),{requestInit:{redirect:'error'}});rec.endpoint=e.mcp.url;}
  ...
  transport=new StdioClientTransport({command:'/usr/bin/sandbox-exec',args:['-f',w+'/sandbox.sb',bin,...extra],env:vars,cwd:scratch,stderr:'pipe'});rec.artifactSha256=j.artifactSha256;rec.lockSha256=j.resolvedLockSha256;
 }
