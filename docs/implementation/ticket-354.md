...
Final audit findings: the memory-projection case passes alone (1 passed) but fails within its module due to NumPy import order; module result is 22 passed/1 failed. Agent panel and recipe round-trip tests passed (9 tests). The recipe inspector accepts model IDs and characters inherit the active chat model, and the static `MODEL_OPTIONS` list now includes all seven native models with provider-qualified values. Seven new real selector interaction tests pass, and all seven values resolve through the sidecar to the matching native provider/model. DeepSeek uses the existing recipe-default inheritance path, which retains its injected custom provider configuration; do not offer it as a standalone native override. Existing graph factory-isolation tests cover this shared inheritance boundary. Model × agent-builder static/contract verdict: **compatible**; live execution remains unverified. No new UI primitives or schemas were added.

Full i18n scan: 815 English keys, ten translated locales, no missing/orphan/mismatched placeholders or missing-in-English code keys; 65 existing dead-key candidates and 48 dynamic call sites were reported, with no deletion. Static rules: no new IPC, storage writes, routing, context providers, overlays or Electron test pairs. Audit overall remains FAIL/incomplete because real-app/artifact acceptance remains incomplete. Total frontend coverage is 41 passing tests (25 provider, nine existing builder/round-trip, seven new model selector cases).
