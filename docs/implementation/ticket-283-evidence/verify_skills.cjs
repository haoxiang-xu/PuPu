/* Live, opt-in evidence: node docs/implementation/ticket-283-evidence/verify_skills.cjs
...
const scratch = fs.mkdtempSync(path.join(os.tmpdir(), 'ticket-283-skills-'));
...
const selected = curation.skillPacks.filter((pack) => [
  'skillpack.vercel-web-review', 'skillpack.trailofbits-audit-prep',
].includes(pack.id));
...
(async () => {
  ...
  for (const entry of selected) {
    ...
    assert.deepEqual(Object.keys(pack).sort(), [
      'degraded', 'rejected', 'skills', 'skipped', 'toolkitId', 'toolkitName',
    ]);
  ...
  const evidence = { checkedAt: new Date().toISOString(), candidateBase: '65811ee3d7bb62cc86f27c681764d7de0bfeef48',
    boundary: 'BC-283-002', sequence: 'SEQ-283-001', checks: ['AC-283-003', 'AC-283-004'], records };
