...
The Flask sidecar runs on `127.0.0.1`, port range `5879-5895`, with a per-session auth token (256-bit hex) passed via `x-miso-auth` header.
...
```
x-miso-auth: <256-bit-hex-token>
```

The token is generated by the Electron main process at startup and passed to both the Flask sidecar and the IPC layer.
