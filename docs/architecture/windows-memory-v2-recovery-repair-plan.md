...
（2026-09-06 补充：PuPu 又追加 `833b6c39`（Task 10 崩溃矩阵测试）、`5c7e2654`（K1/K2/K3 修复）、`2f2ac8cb`（文档）；unchain 追加 `95aeab0`（Codex 第十八节提供的 Windows-only 命名互斥测试，在 Mac 上提交）。K4 修复后的最新 pair 见验收记录第二十一节。）
...
- **BC-009**（canonical 接受 → host receipt/guard）：canonical 接受记录**就是**既有 `interaction.resolved` 事件 + 其 `content_ref` artifact，**没有新建事件 schema**。原子性由新增 `BoundExecutionJournal.append_with_artifacts(*, request, artifacts: tuple[PendingArtifact,...], precondition=None)` 保证：同一个 `BEGIN IMMEDIATE` 连接内，先做 exact operation replay 判定（重放命中则跳过 precondition 与 artifact 重新声明，只做 replay 一致性校验），未命中则在同一连接上执行 `precondition(in_transaction_snapshot)`（拒绝则整体回滚、不留痕迹），再写 artifact 行，最后写事件行。
  ...
  - **接受事实与取消事实的模型（2026-09-06，回应 Codex 第十八节接续要求第 2 条）**：两种事实都落成同一个 `(attempt, "interaction.resolved", interaction_id)` operation 身份下的 canonical 事件，只靠 payload 区分——真实答案的 `response` 是规范化后的用户回答、`submitted_by == "user"`；取消事实的 `response == {"cancelled": true, "reason": ...}`、`submitted_by == "runtime:cancel_execution"`。host 侧对应的是 receipt（不可覆盖，`record_interaction_receipt` 对不同内容抛 `InteractionReceiptConflictError`）与 `application.applied_checkpoint_id`（真实应用为 checkpoint id，取消为 `cancelled:<checkpoint>`）。**裁定规则**：同一会话的接受与取消由 `_interaction_commit_lock(session_id)`（每会话跨进程文件锁，`durable_interaction_commit_locks/<digest>.lock`）串行化——(a) `record_interaction_receipt` 在无副作用预检之后取锁，锁内重读 execution-control 状态、取消 tombstone 与 host receipt/application；已取消则以 `execution_cancelled` 拒绝，什么都不写；否则在锁内依次完成 canonical 原子接受（precondition 仍检查未解决 + 图终态）、host receipt CAS、guard 关联；(b) `cancel_chat_execution` 在取锁后、调用 `cancel_pending` 之前，先查 canonical 是否已有接受事实，有而 host 无 receipt 则先把该答案投影为 host receipt，再让 `cancel_pending` 记录 `cancelled:` application（保留答案、禁止继续），锁在 host 侧取消事实落盘后释放；(c) 之后的 canonical 取消协调如发现 canonical 已有 resolution，必须核对 host receipt 与之一致（缺失则投影，不一致则报 `interaction_cancel_projection_conflict` 而不是宣称 reconciled）。结果矩阵：取消先赢 → canonical 与 host 都是取消标记、答案不落任何一侧；接受先赢 → canonical 与 host 都是答案、application 标记取消、不可继续；崩溃在 canonical 之后 host 之前 → 下一个 `get_pending_interaction` 或下一次取消在锁内把 canonical 答案投影回 host。与 §10.2 第 4 条的关系：没有改造既有 `interaction.resolved` 的语义，取消标记是同一事件类型下的另一种 payload，消费端按 payload 区分。
  - Manifest feature：`durable_interaction` 协议新增 `interaction_resolution_atomic_acceptance_v1`，已同步 unchain `runtime_protocol.py` 与 PuPu 全部三个独立 strict consumer（`context_memory_v2_capability.py` / `electron/main/services/unchain/memory_v2_rollout.js` / `scripts/release-qa/unchain-artifact.mjs`）及其各自测试固件（含 `docs/contracts/memory-v2/windows-required-protocol-and-sink-contract.v1.json`、两个 Electron 本地 fixture manifest）。
...
| cancel 与 accept 先后顺序（含图 resume 后第二问）；**accept/cancel 并发重叠（K4）** | **PASS**（2026-09-06 三次更正：K4 由提交锁根治，见验收记录第二十一节） | 2026-09-06 首次追加时范围收窄到图第一个 interaction，被 Codex 真实 Windows 验收（K2/K3）指出这一收窄不成立——第二问的 cancel 协调本就是原计划要求范围。已在验收记录第十九节按根因修复：(1) `_consume_cancelled_session_guard` 对 `_reconcile_durable_interaction_session_guard` 传 `allow_resolved=True`（取消清理已写入的终态 resolution 是合法的"已解决未 resume"状态，非竞争答案）；(2) `_reconcile_cancelled_interaction_to_context` 用 `pupu_unchain_cold_accepted_interaction_resolution` 先判断是否已有真实 canonical resolution，有则直接返回已完成，不再尝试写入注定冲突的竞争事件。`test_second_interaction_can_be_cancelled[False]`/`[True]`、`test_first_accepted_interaction_cancel_succeeds` 均 PASS，第一个 interaction 的 `test_cancel_before_accept_rejects_the_answer`/`test_accept_before_cancel_keeps_the_accepted_fact`（后者已更新断言为"取消优雅成功"）保持 PASS |
