...
| `BC-ACT-014` | protected signing Environment → reusable package workflow | `CLOSED + LEAST-PRIVILEGE`。正式 RC 的 macOS 与 Windows caller 必须显式 `secrets: inherit`；Windows 独占 OIDC `id-token: write`。Linux caller 独立、不继承 secrets，且只进入 `release-qa` Environment。 | macOS/Windows 缺少任一签名凭据时在 build 前失败；Linux 继承签名 secrets、进入 `release-signing` 或获得 OIDC 时静态契约测试失败。 |
...
| `AC-ACT-017` | workflow 静态测试证明 macOS/Windows caller 显式继承 protected Environment secrets，Linux 无 secret inheritance、无 OIDC 且不进入 `release-signing`；hosted RC 中三路 credential preflight 均按各自平台通过。 | workflow contract tests + hosted RC package logs |
...
| 2026-08-28 | Formal signing secret propagation | local / uncommitted | LOCAL PASS / HOSTED PENDING | `BC-ACT-014`、`AC-ACT-017`：正式 macOS/Windows reusable package caller 显式继承 protected Environment secrets；Linux 拆为独立 caller，不继承 secrets、不获得 OIDC 且不进入 `release-signing`。相关契约测试 10/10、Release QA unit 166/166、long-run harness 62/62、全部 workflow YAML unique-key parse 与 `git diff --check` 通过。该边界没有 durable retry state，故新增 stateful sequence 为 `N/A`；每次 rerun 均重新经过 Environment approval 与 credential preflight。 |
