...
DEVELOPER_PROMPT_SECTIONS = {
    ...
    "delegation": (
        ...
        "delegate when the target lists delegate.\n"
        "- 2+ independent tasks → use spawn_worker_batch only when every "
        "task's target lists worker; otherwise delegate supported targets "
        "sequentially or proceed directly.\n"
        "\n"
        ...
        "{{SUBAGENT_LIST}}\n"
        "\n"
        "Each subagent entry lists its allowed modes. Treat that list as "
        "authoritative.\n"
        "\n"
        ...
        "\n"
        "Calls:\n"
        "- Use delegate_to_subagent(...) only for a target listing delegate.\n"
        "- Use handoff_to_subagent(...) only for a target listing handoff.\n"
        "- Use spawn_worker_batch(...) only when every selected target lists "
        "worker.\n"
        "- Do not attempt an unsupported mode and then retry after runtime "
        "rejection.\n"
        "\n"
