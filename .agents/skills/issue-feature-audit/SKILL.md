...
**Plumbing:** before reading or changing a Project field, follow the
[shared board rules](../release-open-sprint/references/board-api.md): run the project-scope preflight,
...
1. `node .agents/skills/issue-feature-audit/scripts/audit.mjs --root <repo-root> > /tmp/i18n-report.json` (add `--strict` only on request — noisy). Report contains per-locale `missing/orphan/placeholderMismatch` + `code.missingInEn/deadKeys/dynamicCount`.
2. Auto-fill `missing` (the ONLY auto-apply): translate each missing key's en value (preserve `{placeholder}` tokens verbatim), write flat JSON map, then `node .agents/skills/issue-feature-audit/scripts/apply.mjs --root <repo-root> --locale <name> --translations /tmp/<name>.json`. Never overwrites existing values.
...
Keep the durable `release-feature-audit:v2` and `release-audit-waiver:v2` markers unchanged so existing audit records remain discoverable after this skill rename.
...
## Issue feature audit — YYYY-MM-DD
Release: #PARENT
Overall: PASS | FAIL
...
- Running i18n scripts from retired project-local paths — use this skill's scripts directory.
