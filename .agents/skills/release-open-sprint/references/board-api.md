...
This delegation includes the refinement completion transition below. It does
not authorize a change to the intended
user result, release scope, non-goals, required capability, title, labels,
Size, other Project Status transitions, Iteration, assignee, parent/child relationship, Release
...
After a complete researched brief is saved and verified, automatically move an
open delivery issue from Planning to Ready for Pickup if it is a direct child
of an open Release and no unresolved scope decision or dependency prevents
pickup. No additional project owner confirmation is required. Read the current
Status before updating; Ready for Pickup is already complete, and Assigned,
In Progress, In Review, or Done must be preserved. Incomplete or blocked
refinement retains its current Status, with the reason reported. This exception
changes only Status, never the assignee or other fields, and applies to both
standalone refinement and refinement within issue-start-ticket.
