...
const UPDATE_EVENT_CHANNEL = "update:state-changed";
...
const emitAppUpdateState = () => {
  ...
  targets.forEach((target) => {
    ...
    try {
      target.send(UPDATE_EVENT_CHANNEL, payload);
    } catch (_) {
...
ipcMain.handle("update:get-state", () => getAppUpdateStatePayload());
ipcMain.handle("update:check-and-download", async () =>
  checkAndDownloadAppUpdate(),
);
ipcMain.handle("update:install-now", async () => installDownloadedAppUpdate());
